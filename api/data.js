...
async function handlePortalSync(req, res) {
  ...
  try {
    const upstream = await fetch(bridgeUrl, {
      ...
      body: JSON.stringify({ target: 'coverageDesk', secret, resource: resourceRequested, month }),
    });
    ...
    return res.status(200).json({ ok: true, data: parsed.data, generatedAt: parsed.generatedAt || null });
  } catch (err) {
