  ...
  if (req.method === 'POST') {
    ...
    try {
      ...
      if (rows.length) {
        ...
        // status/statusFields/interviewer/importance were added later in the
        // app's life and are now included here — they were previously being
        // silently dropped on save because this list never mentioned them,
        // even though the SQL insert itself succeeded without error.
        const values = rows.map(r => [
          ...
          r.status||'', JSON.stringify(r.statusFields||[]), r.interviewer||'', r.importance||''
        ]);
        await conn.query(
          `INSERT INTO calls (id, call_date, time_text, candidate, company, round_text, duration, is_woi, assignee, country, doubts_json, raw_text, status, status_fields_json, interviewer, importance)
           VALUES ?`,
          [values]
        );
      }
