...
module.exports = async (req, res) => {
  ...
  if (resource === 'ping') {
    try {
      ...
      return res.status(200).json({ ok: true, ts: Date.now(), version: 'reschedule-via-calls-v4' });
    } catch (e) {
  ...
  if (req.method === 'POST') {
    ...
    try {
      ...
      if (rows.length) {
        ...
        // status/statusFields are now part of this same proven save path,
        // instead of a separate isolated table — reuses exactly the same
        // reliable INSERT that assignee/company/etc. already go through.
        const values = rows.map(r => [
          ...
          r.interviewer||'', r.importance||'', r.status||'', JSON.stringify(r.statusFields||[])
        ]);
        await conn.query(
          `INSERT INTO calls (id, call_date, time_text, candidate, company, round_text, duration, is_woi, assignee, country, doubts_json, raw_text, interviewer, importance, status, status_fields_json)
           VALUES ?`,
          [values]
        );
      }
...
function rowToCall(row) {
  return {
    ...
    status: row.status || '',
    statusFields: safeParse(row.status_fields_json, []),
  };
}
